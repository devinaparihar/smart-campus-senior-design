...
for split in range(N_SPLITS):
    ...
    for i in range(N_EPOCHS):
        ...
        with torch.no_grad():
            ...
            for j, n in enumerate(test_steps):
                ...
                avg_prob = np.mean(np.array([forward, forward2]), axis=0)
                output3 = np.argmax(avg_prob, axis=1) #ensembled prediction
    ...
    print("Mean Accuracies3 (ensembled): {}".format(mean_accuracies3 / (split + 1)))
